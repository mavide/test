...
var loading = loading_img+" Loading...";
	...
	function getAjaxCollection(type,div)
	{
		$(div).css("display","block");
		var preservedHTML = $(div).html();
		$.ajax({
			...
			type : 'POST',
			dataType : 'json',
			data : ({ mode : 'loadAjaxCollections', 'collectionsType' : type }),
			beforeSend : function ()
			{
				$(div).html(loading);	
			},
			success : function (data)
			{
				if(data['failed'])
				{
					//alert("No Collections Returned");
					$(div).html(preservedHTML);
				}
					
				if(data['completed'])
				{	
					$(div).html(data['collectionBlocks']);
				}
			}
		})
	}

	function getAjaxArtist(type,div)
	{
		$(div).css("display","block");
		var preservedHTML = $(div).html();
		$.ajax({
			...
			type : 'POST',
			dataType : 'json',
			data : ({ mode : 'loadAjaxArtists', 'artistsType' : type }),
			beforeSend : function ()
			{
				$(div).html(loading);	
			},
			success : function (data)
			{
				if(data['failed'])
				{
					//alert("No Artists Returned");
					$(div).html(preservedHTML);
				}
					
				if(data['completed'])
				{	
					$(div).html(data['artistBlocks']);
				}
			}
		})
	}
...
function artist_actions(form,mode,objID,result_con,type,cid)
{
	$(result_con).css("display","block");
	$(result_con).html(loading);
