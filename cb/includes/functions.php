...
{
	$patterns = explode("\r\n",file_get_contents(BASEDIR .'/censor_words.txt'));
...
function search_bad_words($input) {
	...
	$phrases = explode("\r\n", file_get_contents(BASEDIR .'/blacklist.txt'));
	...
	function getCategoryList($params=false)
	{
		...
		switch($type)
		{
...

			{
				...
				$cats = $cbartist->cbCategories($params);
...
function call_view_artist_functions($cdetails)

	{

		$funcs = get_functions('view_artist_functions');
		...
		increment_views($cdetails['artist_id'],"artist");
	...
	function increment_views($id,$type=NULL)
	{
		...
		switch($type)
		{
...

			{
				...
				if(!isset($_COOKIE['artist_'.$id])){

					$db->update(tbl("artists"),array("views"),array("|f|views+1")," artist_id = '$id'");

					setcookie('artist_'.$id,'viewed',time()+3600);
	...
	function show_collection_form($params)
	{
		...
		/**
	 * Function used to show artist form
	 */
	function show_artist_form_cl($params)
	{
		global $db,$cbartist;
		if(!userid())
			$loggedIn = "not";
		...
		{		
			$collectArray = array("order"=>" artist_name ASC","type"=>"videos","user"=>userid());		
			$artists = $cbartist->get_artists($collectArray);
			
			assign("artists",$artists);
		}
		Template("/blocks/artist_form_cl.html");	
	}
	
		function show_artist_form_v($params)
	{
		global $db,$cbartist;
		if(!userid())
			$loggedIn = "not";
		...
		Template("/blocks/artist_form_v.html");	
	}
	 ...
	 * function used to get photos
	 */
	function get_artists($param)
	{
		global $cbartist;
		return $cbartist->get_artists($param);
	}
	 ...
	 * function used to get photos
	 */
	function get_public_active_collections($param)
	{
		global $cbcollection;
		return $cbcollection->get_public_active_collections($param);
	}
		function get_public_active_artists($param)
	{
		global $cbartist;
		return $cbartist->get_public_active_artists($param);
	}
	// FIN MDF V00.2
	/**
	 * function used to get vidos
