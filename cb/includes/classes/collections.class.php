...
// MDF V00.2	
		/**
	 * Function used to get active and public collections
	 */
	function get_public_active_collections($p=NULL)
	{
		global $db;
...
// FIN MDF V00.2	
	 ...
	 * @return : Object
	 */
	function getPublicCollectionFromItem($objId,$type=NULL)
	{
		global $db;
		if(!$type)
			$type = $this->objType;
		...
		$results = $db->select(tbl('collections,collection_items'),'*',
		tbl("collections.collection_id")." = ".tbl("collection_items.collection_id")." AND "
		.tbl("collection_items.type='".$type."'")." AND ".tbl("collections.broadcast='".$broadcast."'")." AND "
		.tbl("collections.active='yes'")." AND ".tbl("collection_items.object_id='".$objId."'"));
