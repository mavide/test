...
if(!empty($mode))
{
	switch($mode)
	{
		...
		case 'add_to_fav':
		{
			...
			switch($type)
			{
...

				{
					...
					$cbartist->action->add_to_fav($id);

					//updateObjectStats('fav','artist',$id); // Increment in total favs
					...
					$funcs = cb_get_functions('favorite_artist');	
		...
		case "add_new_item_ar":
		{
			...
			switch($type)
			{
				...
				case "cl":
				{
					...
					$cid = $_POST['cid'];
					$id = $_POST['obj_id'];
					$cbvideo->artist->add_artist_item_cl($id,$cid,$type);	
				}
				break;
				...
				default:
				{
					...
					$cid = $_POST['cid'];
					$id = $_POST['obj_id'];
					$cbvideo->artist->add_artist_item_v($id,$cid,$type);	
				}
			}
		...
		case "loadAjaxCollections":
		{
			$collectionsType = $_POST['collectionsType'];
			$cond = array("limit"=>config("collection_home_tabs"));
			switch($collectionsType)
			{
				case "col_last_viewed":
				default:
				{
					$cond['order'] = " last_viewed DESC";	
				}
				...
				case "col_most_favorited":
				{
					$cond['order'] = " total_favorites DESC";
				}
				break;
				
				case "col_most_commented":
				{
					$cond['order'] = " total_comments DESC";
				}
				break;
				
				case "col_highest_rated":
				{
					$cond['order'] = " rating DESC, rated_by DESC";
				}
				break;
				
				case "col_most_viewed":
				{
					$cond['order'] = " views DESC";
				}
				break;
				
				case "col_most_downloaded":
				{
					$cond['order'] = " downloaded DESC";
				}
				break;
			...
			$collections = get_collections($cond);
			if($collections)
			{
				foreach($collections as $collection)
				{
					assign("collection",$collection);
					$cond['collectionBlocks'] .= Fetch("/blocks/collection.html");	
				}
				$cond['completed'] = "successfull";
			} else {
				$cond['failed'] = "successfully";	
		...
		case "loadAjaxArtists":
		{
			$artistsType = $_POST['artistsType'];
			$cond = array("limit"=>config("artist_home_tabs"));
		switch($artistsType)
		{
		case "ar_last_viewed":
		default:
		...
		case "ar_most_commented":
		{
		$cond['order'] = " total_comments DESC";
		}
		break;

		case "ar_highest_rated":
		{
		$cond['order'] = " rating DESC, rated_by DESC";
		}
		break;
		...
		break;
			...
			$artists = get_artists($cond);
			if($artists)
			{
				foreach($artists as $artist)
				{
					assign("artist",$artist);
					$cond['artistBlocks'] .= Fetch("/blocks/artist.html");	
				}
				$cond['completed'] = "successfull";
			} else {
				$cond['failed'] = "successfully";	
