...
function get_thumb_link($video_data)
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		if (preg_match('/defaulthumbnailhref="(.*?)"/', $video_data[$i], $matches) != 0)
		{
			return $matches[1];
		}
	}
...
function video_details($video_data, $url, &$info)
{
	...
	for($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $thumb)
		{
			if (preg_match('/<meta property="og:image" content="(.*?)"/', $video_data[$i], $matches) != 0)
			{
		...
		if ( ! $xml_url)
		{
			if (strpos($video_data[$i], 'flashvars ') !== false)
			{
				for ($j = 1; $j <= 10; $j++)
				{
					if (preg_match('/videolist: "(.*?)"/', $video_data[$i + $j], $matches) != 0)
					{
						$xml_url = $matches[1];
						break;
					}
