...
function get_info($url)
{
	...
	if (preg_match('/peteava\.ro\/id-(.*?)-/', $url, $matches) != 0)
	{
		$target_url = 'http://www.peteava.ro/id-'. $matches[1];
	}
	else 
	{	
		$target_url = $url;
...
function video_details($video_data, $url, &$info)
{
	...
	if (strpos($info['video_title'], '-') !== false)
	{
		$pieces = explode('-', $info['video_title'], 2);
	...
	$info['url_flv'] = 'http://content.peteava.ro/stream.php&file='. $info['yt_id'] .'_standard.mp4';
	$info['yt_thumb'] = 'http://storage2.peteava.ro/serve/thumbnail/'. $info['yt_id'] .'/playerstandard';
}
