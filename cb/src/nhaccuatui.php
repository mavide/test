...
function video_details($video_data, $url, &$info)
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		if (($title && $thumb && $tags && $description && $swf) || strpos($video_data[$i], '<body') !== false)
		{
			break;
		}
	}
	...
	if (preg_match('/mv4u\/xem-clip\/(.*?)\//', $url, $matches) != 0)
	{
		$info['yt_id'] = $matches[1];
		
		if ($info['url_flv'] == '')
		{
			$info['url_flv'] = 'http://www.nhaccuatui.com/mv/xem-clip/'. $matches[1];
		}
	}
	else
	{
		preg_match('/\?(L|M)=([^(\&|$)]*)/', $url, $matches);
		$info['yt_id'] = $matches[2];
		
		if ($info['url_flv'] == '')
		{
			$info['url_flv'] = 'http://www.nhaccuatui.com/'. strtolower($matches[1]) .'/'. $matches[2];
		}
	}
}
