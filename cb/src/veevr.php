...
function video_details($video_data, $url, &$info)
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		if (($title && $thumb && $duration && $direct && $tags && $description) || strpos($video_data[$i], '<body') !== false)
		{
			break;
		}
	}
	...
	if ($info['direct'] == '')
	{
		$info['direct'] = 'http://veevr.com/videos/'. $id;
	}
	
	if ($info['url_flv'] == '')
	{
		$info['url_flv'] = 'http://veevr.com/embed/'. $info['yt_id'];
	}
}
