...
function video_details($video_data, $url, &$info) 
{
	...
	$matches = array();
	$embed_code = '';
	$swf = $artist = $title = $thumb = $duration = false;
	...
	for($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $artist)
		{
			if (strpos($video_data[$i], '"audio_artist"'))
			{
				if (preg_match('/content="(.*?)"/', $video_data[$i], $matches) != 0)
				{
		...
		if ( ! $title)
		{
			if (strpos($video_data[$i], '"audio_title"'))
			{
				if (preg_match('/content="(.*?)"/', $video_data[$i], $matches) != 0)
				{
		...
		if ( ! $duration)
		{
			if (strpos($video_data[$i], '"music:duration"'))
			{
				if (preg_match('/content="(.*?)"/', $video_data[$i], $matches) != 0)
				{
		...
		if (($artist && $title && $thumb && $duration && $direct && $tags && $description && $swf) || strpos($video_data[$i], '<body') !== false)
		{
			break;
		}
	}
