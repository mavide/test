...
function get_info($url)
{
	...
	$target_url = "http://vimeo.com/api/v2/video/" . $vid_id .'.php';
	...
	else if(ini_get('allow_url_fopen') == 1)
	{
		...
		{
    		echo '<div class="alert alert-danger"><strong>Vimeo</strong> requires a PHP extension that is not installed. Ask your hosting provider to enable <strong>cURL</strong></div>';
    	}    
...
function get_flv($video_data, $mobile_url = false)
{
	...
	$time = '';
	$v_id = $video_data['id'];
	$q = ($mobile_url) ? 'mobile' : 'sd';
	
	$target_url = 'http://player.vimeo.com/config/'.$v_id .'?type=moogaloop&referrer=&player_server=player.vimeo.com&v=1.0.0&cdn_server=a.vimeocdn.com&width=1920&height=891';
	...
	preg_match('/"timestamp":(.*?),/', $data, $matches);
	$time = $matches[1];
	preg_match('/"signature":"(.*?)",/', $data, $matches);
	$sig = $matches[1];
		
	$target_url  = 'http://player.vimeo.com/play_redirect?clip_id='. $v_id .'&sig='. $sig .'&time='. $time .'&quality='.$q.'&codecs=H264,VP8,VP6&type=moogaloop&embed_location=';
	unset($data);
	if(function_exists('curl_init'))
	{
		$ch = curl_init();
