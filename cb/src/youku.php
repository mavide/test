...
function get_info($url)
{
	...
	preg_match('/youku\.com\/v_show\/id_(.*?)\.html/', $url, $matches);
	
	$video_id = $matches[1];

	$target_url  = 'http://v.youku.com/v_show/id_'. $video_id .'.html';
...
function get_thumb_link($video_data, $url) 
{
	...
	for($i = 0; $i < $arr_length; $i++)
	{
		...
		if(preg_match('/do_addprofilevideo\.php\?vid=(.*?)&img=(.*?)">/', $video_data[$i], $matches) != 0)
		{
			$thumb_link = urldecode($matches[2]);
			break;
		}
	}
...
function video_details($video_data, $url, &$info) 
{
	...
	for($i = 0; $i < $arr_length; $i++)
	{
		...
		if(preg_match('/<embed(.*)<\/embed>/i', $video_data[$i], $matches) != 0)
		{
	...
	//	direct url
	$info['direct'] = 'http://v.youku.com/v_show/id_'. $matches[1] .'.html';
