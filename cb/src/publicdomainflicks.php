...
function get_flv($video_data)
{
	...
	for ($i = 50; $i < $arr_length; $i++)
	{
		if (preg_match('/"file","(.*?)"/', $video_data[$i], $matches) != 0)
		{
...
function video_details($video_data, $url, &$info)
{
	...
	preg_match("/publicdomainflicks\.com\/([0-9]+)-/", $url, $matches);
	$info['yt_id'] = $matches[1];
	$info['yt_thumb'] = 'http://www.publicdomainflicks.com/img/'. $info['yt_id'] .'.jpg';
	$info['url_flv'] = get_flv($video_data);
	
	$ext = strtolower(array_pop(explode('.', $info['url_flv'])));
	
	if ($ext == 'mp4')
	{
		$info['mp4'] = $info['url_flv'];
	}
}
