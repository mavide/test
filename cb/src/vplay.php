...
function get_info($url)
{
	...
	$url = rtrim($url, '/');
	$pieces = explode('/', $url);
	$target_url = 'http://vplay.ro/watch/'. $pieces[count($pieces) - 1] .'/';
...
function video_details($video_data, $url, &$info)
{
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		{
			$info['video_title'] = str_replace('Vplay /', '', $matches[1]);
	...
	$info['direct'] = 'http://vplay.ro/watch/'. $id;
	$info['url_flv'] = 'http://i.vplay.ro/f/embed.swf?key='. $id;
	$info['yt_id'] = $id;
	$info['yt_thumb'] = 'http://i.vplay.ro/th/'. $prefix .'/'. $id .'/0.jpg';
