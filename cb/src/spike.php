...
function video_details($video_data, $url, &$info) 
{
	...
	$matches = array();
	$embed_code = '';
	$title = $direct = $tags = $description = false;
	...
	for($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $direct)
		{
			...
			if (strpos($video_data[$i], '"canonical"'))
			{
		...
		if ( ! $tags)
		{
			if (strpos($video_data[$i], '"og:tag"') || strpos($video_data[$i], '"keywords"'))
			{
				if (preg_match('/content="(.*?)"/', $video_data[$i], $matches) != 0)
				{
					$info['tags'] = $matches[1];
		...
		if (($title && $direct && $tags && $description && $swf) || strpos($video_data[$i], '<body') !== false)
		{
			break;
		}
	}
